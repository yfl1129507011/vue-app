<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div @click="changeDefault(addr)" class="address clearFix" v-for="addr in addrInfo" :key="addr.id">
        ...
        <p>
          ...
          <span v-show="addr.isDefault == 1" class="s3">默认地址</span>
        </p>
      </div>
      <div class="line"></div>
      ...
      <div class="detail">
        ...
        <ul class="list clearFix" v-for="order in orderInfo.detailArrayList" :key="order.skuId">
          <li>
            <img :src="order.imgUrl" style="width:50px;height:50px" alt="">
          </li>
          <li>
            <p>{{ order.skuName }}</p>
            <h4>7天无理由退货</h4>
      ...
      <div class="bbs">
        <h5>买家留言：</h5>
        <textarea v-model="comment" placeholder="建议留言前先与商家沟通确认" class="remarks-cont"></textarea>
...
export default {
  ...
  computed: {
    ...mapState({
      addrInfo: (state) => state.trade.addrInfo || [],
  ...
  methods: {
    changeDefault(addr) {
      this.addrInfo.forEach(item => item.isDefault = 0)
      addr.isDefault = 1
    },
    async submitOrder() {
      ...
      let res = await this.$API.reqTradeInfo(tradeNo, tradeInfo)
      if (res.code == 200) {
